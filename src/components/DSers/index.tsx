...
import {
  ...
  RetrieveProducts,
  FuncRetrieve,
} from '../../DSers/Core/Core';

import { HandlingImageRequest } from '../../DSers/Core/Utils';

import { DeleteSurplusData } from '../../DSers/Core/Utils/Surplus';

import { getApiKey, getVerifyStore } from '../../DSers';
...
const DSers = () => {
  ...
  const getKey = async () => {
    try {
      console.log(await getApiKey('https://atc.dsers.com/', 'c9842lspuf6g01gkrtmg'));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const verifyStore = async () => {
    try {
      console.log(await getVerifyStore('https://atc.dsers.com/', 'https://neuraleague.com'));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <>
      ...
      <Button variant="contained" onClick={getKey}>
        getApiKey
      </Button>{' '}
      <Button variant="contained" onClick={verifyStore}>
        getVerifyStore
      </Button>
    </>
  );
